defmodule Boltx.Internals.BoltProtocolV1 do
  @moduledoc false
  alias Boltx.Internals.BoltProtocolHelper
  ...
  See "Shared options" in `Boltx.Internals.BoltProtocolHelper` documentation.
  ...
  @spec handshake(atom(), port(), Keyword.t()) ::
          {:ok, integer()} | {:error, Boltx.Internals.Error.t()}
  def handshake(transport, port, options \\ [recv_timeout: 15_000]) do
    ...
    Boltx.Internals.Logger.log_message(
      :client,
      :handshake,
    ...
    case transport.recv(port, 4, recv_timeout) do
      {:ok, <<version::32>> = packet} when version <= max_version ->
        Boltx.Internals.Logger.log_message(:server, :handshake, packet, :hex)
        Boltx.Internals.Logger.log_message(:server, :handshake, version)
        {:ok, version}
  ...
  ## Examples
      ...
      iex> Boltx.Internals.BoltProtocol.init(:gen_tcp, port, 1, {}, [])
      {:ok, info}

      iex> Boltx.Internals.BoltProtocol.init(:gen_tcp, port, 1, {"username", "password"}, [])
      {:ok, info}
  """
  @spec init(atom(), port(), integer(), tuple(), Keyword.t()) ::
          {:ok, any()} | {:error, Boltx.Internals.Error.t()}
  ...
  @spec pull_all(atom(), port(), integer(), Keyword.t()) ::
          {:ok, list()}
          | {:failure, Boltx.Internals.Error.t()}
          | {:failure, Boltx.Internals.Error.t()}
  ...
  ## Examples
      ...
      iex> Boltx.Internals.BoltProtocol.run_statement(:gen_tcp, port, 1, "MATCH (n) RETURN n")
      [
        {:success, %{"fields" => ["n"]}},
  ...
  ## Example
      ...
      :ok
  """
  @spec discard_all(atom(), port(), integer(), Keyword.t()) :: :ok | Boltx.Internals.Error.t()
  ...
  ## Example
      ...
      :ok
  """
  @spec ack_failure(atom(), port(), integer(), Keyword.t()) :: :ok | Boltx.Internals.Error.t()
  ...
  ## Example
      ...
      :ok
  """
  @spec reset(atom(), port(), integer(), Keyword.t()) :: :ok | Boltx.Internals.Error.t()
