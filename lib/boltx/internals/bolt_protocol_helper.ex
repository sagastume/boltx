defmodule Boltx.Internals.BoltProtocolHelper do
  @moduledoc false
  ...
  """
  @spec send_message(atom(), port(), integer(), Boltx.Internals.PackStream.Message.raw()) ::
          :ok | {:error, any()}
  ...
  * `:failure`
  ...
  @spec receive_data(atom(), port(), integer(), Keyword.t(), list()) ::
          {atom(), Boltx.Internals.PackStream.value()}
          | {:error, any()}
  ...
  @spec treat_simple_message(
          Boltx.Internals.Message.out_signature(),
          atom(),
          port(),
          integer(),
          Keyword.t()
