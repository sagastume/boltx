defmodule Boltx.Internals.PackStream.Message.Encoder do
  @moduledoc false
  _module_doc = """
  ...
  All messages are serialized structures. See `Boltx.Internals.PackStream.EncoderV1` for
  ...
  @spec signature(Boltx.Internals.PackStream.Message.out_signature()) :: integer()
  ...
  """
  def client_name() do
    "boltx/" <> to_string(Application.spec(:boltx, :vsn))
  end
  ...
  Examples (excluded from doctest because client_name changes at each boltx version)
  ...
  Example
      ...
      iex> alias Boltx.Internals.PackStream.Message.Encoder
  ...
  Examples (excluded from doctest because client_name changes at each boltx version)
      ...
      # without auth token
      diex> alias Boltx.Internals.PackStream.Message.Encoder
  ...
  """
  @spec encode_message(
          Boltx.Internals.PackStream.Message.out_signature(),
          integer(),
          list(),
          integer()
        ) ::
  ...
  def encode_message(message_type, signature, data, bolt_version) do
    ...
    encoded =
      {signature, data}
      |> Boltx.Internals.PackStream.encode(bolt_version)
      |> generate_chunks([])
  ...
  @spec generate_chunks(Boltx.Internals.PackStream.value() | <<>>, list()) ::
          [[Boltx.Internals.PackStream.Message.encoded()]]
  ...
  @spec format_chunk(Boltx.Internals.PackStream.value()) ::
          [Boltx.Internals.PackStream.Message.encoded()]
