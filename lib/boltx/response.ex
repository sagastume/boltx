defmodule Boltx.Response do
  @moduledoc """
  Support for transforming a Bolt response to a list of Boltx.Types or arbitrary values.
  A Boltx.Response is used for mapping any response received from a Neo4j server into a an Elixir struct.
  ...
  For example, a simple Cypher query like this:
      ...
      Boltx.query(Boltx.conn(), "RETURN [10,11,21] AS arr")
  ...
  and while you have now access to the full data returned by Neo4j, most of the times you'll just want the results:
      ...
      iex> %Boltx.Response{results: results} = Boltx.query!(Boltx.conn(), "RETURN [10,11,21] AS arr")
      iex> results
      [%{"arr" => [10, 11, 21]}]
  ...
  will return `results` like this:
      ...
      [%{"r" => %Boltx.Types.Relationship{end: 647, id: 495,
  ...
  Our Boltx.Response is implementing Elixir's [Enumerable Protocol](https://hexdocs.pm/elixir/Enumerable.html), to help you accessing the results. Hence something like this, is possible:

      iex> Boltx.query!(Boltx.conn(), "RETURN [10,11,21] AS arr") |>
  ...
  You can also quickly get the `first` of the results, returned by Neo4j. Example:
      ...
      iex> Boltx.query!(Boltx.conn(), "UNWIND range(1, 10) AS n RETURN n") |>
  ...
  def transform(records, _stats \\ :no) do
    # records |> IO.inspect(label: "raw>> lib/boltx/response.ex:44")
    ...
    e in Boltx.Exception -> {:error, e.message}
    e -> {:error, e}
  end

  def fetch(%Boltx.Response{fields: fields, results: results}, key) do
  ...
  def fetch!(%Boltx.Response{} = r, key) do
